...
const LogIn = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (foundUser) {
        //  conditional rendering since the revisionSurahs is only
        // created if user selects some surah strengths
        if (foundUser.revisionSurahs?.length === 0) {
      ...
      console.error("Error in getting user in login", error)
