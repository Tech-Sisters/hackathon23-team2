...
const Test = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${API_ENDPOINT}/surahs/ayat?surahId=${surah.id}`)
            ...
            <div className="container-fluid py-5 border rounded test-container d-flex flex-column justify-content-between">
              ...
              <div className="row my-3 justify-content-center">
                <div className="col-10 d-flex justify-content-center">
                  <h6 className={`fw-normal text text-wrap text-break ${hideAyah ? "hidden" : "visible"} `}>
              ...
              <div className="row my-3 d-flex justify-content-center m-2">
                <div className="col-10 d-flex justify-content-center">
                  {completeAyah ? (
                    ...
                    <button className="hideButton rounded-pill p-2 mx-2" onClick={handleClickUnhide}>
                      {hideAyah ? "Unhide" : "Hide"}
                    </button>
                  )}
                  {completeAyah ? (
                    <button className="completeButton rounded-pill p-2 mx-2" onClick={handleClickNext}>
                      Next
                    </button>
                  ) : (
                    <button className="completeButton rounded-pill p-2 mx-2" onClick={handleClickComplete}>
                      Complete
                    </button>
                  )}
                </div>
